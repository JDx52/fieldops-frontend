  ...
  return (
    <div style={{ flex:1,display:"flex",flexDirection:"column",overflow:"hidden" }}>
      ...
      <div style={{ flex:1,overflowY:"auto" }}>
        {loading?<Spinner />:list.length===0?<EmptyState icon="👥" title="No customers yet" desc="Add your first customer" action={<Btn onClick={()=>setShowNew(true)}>+ New Customer</Btn>} />:list.map(c=><div key={c.id} style={{ padding:"14px 16px",borderBottom:"1px solid var(--border)",display:"flex",justifyContent:"space-between",alignItems:"center",gap:12,transition:"background .1s" }} onMouseEnter={e=>e.currentTarget.style.background="var(--surface2)"} onMouseLeave={e=>e.currentTarget.style.background="transparent"}><div onClick={()=>setSelected(c)} style={{ flex:1,cursor:"pointer",minWidth:0 }}><div style={{ fontSize:14,fontWeight:600,marginBottom:2 }}>{c.first_name} {c.last_name}</div><div style={{ fontSize:12,color:"var(--text3)" }}>{c.phone||c.email||"No contact"} · {c.job_count||0} job{c.job_count!==1?"s":""}</div></div><div style={{ display:"flex",gap:8,alignItems:"center",flexShrink:0 }}><button onClick={e=>{e.stopPropagation();const msg=encodeURIComponent(`Jayson is trying to get his Google reviews up. If you have time, would you mind leaving a Google review he would really appreciate it. Thanks - 405 Heating and Air Conditioning https://g.page/r/CVTRHVvrhBTBEBM/review`);const phone=(c.phone||c.cell||"").replace(/\D/g,"");if(phone){window.open(`sms:${phone}?body=${msg}`);}else{alert("No phone number for this customer.");}}} style={{ fontSize:11,fontWeight:600,background:"var(--green-lt)",color:"var(--green)",border:"1px solid var(--green-bd)",borderRadius:6,padding:"4px 10px",cursor:"pointer",whiteSpace:"nowrap" }}>⭐ Review</button><span onClick={()=>setSelected(c)} style={{ color:"var(--text4)",fontSize:16,cursor:"pointer" }}>›</span></div></div>)}
      </div>
    </div>
  );
}
...
const PB_KEY = "fieldops_pb_405_v4";
