...
async function saveWorkOrder(wo) {
  try {
    // Encode payment method as a tag in description_of_work so it survives API round-trips on any device
    let desc = (wo.descriptionOfWork||"").replace(/\[PAYMENT:[^\]]*\]/g,"").trim();
    if(wo.paymentMethod){
      const tag = wo.checkNumber ? `[PAYMENT:${wo.paymentMethod}:${wo.checkNumber}]` : `[PAYMENT:${wo.paymentMethod}]`;
      desc = desc ? desc + "\n" + tag : tag;
    }
...
function normalizeWO(w) {
  const desc = w.description_of_work||w.descriptionOfWork||"";
  const payTag = desc.match(/\[PAYMENT:([^\]:]+)(?::([^\]]*))?\]/);
  const payMethod = w.payment_method||w.paymentMethod||(payTag?payTag[1]:null);
