  ...
  async function handleSave() {
    ...
    try {
      const payload = {
        ...
        line_items: items.filter(i=>i.name).map(i=>({
          name: String(i.name),
...
function JobDetailModal({ job, onClose, onUpdated }) {
    ...
    async function load() {
      try {
        const [notesData, photosData] = await Promise.all([
          apiFetch(`/jobs/${job.id}/notes`).catch(() => []),
          apiFetch(`/jobs/${job.id}/photos`).catch(() => []),
        ]);
  ...
  async function addNote() {
    ...
    setSaving(true);
    try {
      const note = await apiFetch(`/jobs/${job.id}/notes`, {
        method: "POST",
        body: JSON.stringify({ content: newNote.trim(), note_type: "general" }),
      });
      setNotes(p => [note, ...p]);
      setNewNote("");
    } catch(e) { alert(e.message); }
  ...
  async function handlePhotoUpload(e) {
    ...
    reader.onload = async (ev) => {
      try {
        // Store as base64 data URL in notes for now
        const photo = await apiFetch(`/jobs/${job.id}/photos`, {
          method: "POST",
          body: JSON.stringify({ 
        ...
        // If photo API fails, just show it locally
        setPhotos(p => [{ id: Date.now(), url: ev.target.result, caption: file.name, created_at: new Date().toISOString() }, ...p]);
      }
    };
  ...
  return (
    <Modal title={`${job.job_number} — ${job.title}`} onClose={onClose} width={600}>
      <div style={{ padding: "0 24px 8px", borderBottom: "1px solid var(--border)", display: "flex", gap: 6 }}>
        <button style={tabStyle("notes")} onClick={() => setTab("notes")}>📝 Notes ({notes.length})</button>
        <button style={tabStyle("photos")} onClick={() => setTab("photos")}>📷 Photos ({photos.length})</button>
      </div>
        ...
        <div style={{ padding: "16px 24px" }}>
            ...
            <>
              ...
              <div style={{ marginBottom: 16 }}>
                <textarea
                  ...
                  placeholder="Add a job note — what did you find? What did you fix? Parts used?"
                  style={{ ...inputStyle, height: 100, resize: "vertical", marginBottom: 8 }}
                />
                <Btn onClick={addNote} disabled={saving || !newNote.trim()} small>
              ...
              {notes.length === 0 ? (
                ...
                <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
                  {notes.map(n => (
                    <div key={n.id} style={{ background: "var(--surface2)", border: "1px solid var(--border)", borderRadius: 8, padding: "12px 14px" }}>
                      <div style={{ fontSize: 13, color: "var(--text1)", lineHeight: 1.6, marginBottom: 6 }}>{n.content}</div>
                      <div style={{ fontSize: 11, color: "var(--text3)" }}>
                        {n.author_name || "Tech"} · {new Date(n.created_at).toLocaleDateString("en-US", { month: "short", day: "numeric", hour: "numeric", minute: "2-digit" })}
                      </div>
                    </div>
                  ))}
                </div>
          ...
          {tab === "photos" && (
            <>
              ...
              {photos.length === 0 ? (
                <div style={{ textAlign: "center", padding: "24px", color: "var(--text3)", fontSize: 13 }}>
                  No photos yet. Tap "Upload Photo" to add one.
                </div>
              ) : (
                <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(160px, 1fr))", gap: 10 }}>
                  {photos.map(p => (
                    <div key={p.id} style={{ borderRadius: 8, overflow: "hidden", border: "1px solid var(--border)", background: "var(--surface2)" }}>
                      <img src={p.url} alt={p.caption || "Job photo"} style={{ width: "100%", aspectRatio: "1", objectFit: "cover", display: "block" }} />
                      <div style={{ padding: "6px 8px", fontSize: 11, color: "var(--text3)" }}>
                        {p.caption || "Photo"} · {new Date(p.created_at).toLocaleDateString("en-US", { month: "short", day: "numeric" })}
                      </div>
                    </div>
                  ))}
                </div>
