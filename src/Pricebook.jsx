...
export default function Pricebook() {
  ...
  function handleSave() {
    ...
    if (editing) {
      setItems(p => p.map(i => i.id === editing ? { ...i, ...form, price: parseFloat(form.price) } : i));
    } else {
      setItems(p => [...p, { id: Date.now(), ...form, price: parseFloat(form.price) }]);
    }
    setShowForm(false);
  }
        ...
        <div style={{ position: "fixed", inset: 0, background: "#00000060", zIndex: 100, display: "flex", alignItems: "center", justifyContent: "center", padding: 20 }} onClick={e => e.target === e.currentTarget && setShowForm(false)}>
          <div style={{ background: "var(--surface)", borderRadius: 16, width: "100%", maxWidth: 480, border: "1px solid var(--border)", boxShadow: "0 24px 64px #00000030" }}>
            ...
            <div style={{ padding: "16px 24px", borderTop: "1px solid var(--border)", display: "flex", justifyContent: "flex-end", gap: 10 }}>
              ...
              <button onClick={handleSave} style={{ background: "var(--blue)", color: "#fff", border: "none", borderRadius: 8, padding: "9px 18px", fontSize: 13, fontWeight: 500, cursor: "pointer" }}>{editing ? "Save Changes" : "Add Item"}</button>
            </div>
          </div>
        </div>
      )}
